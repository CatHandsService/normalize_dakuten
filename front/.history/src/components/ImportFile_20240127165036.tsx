...
export const ImportFile = () => {
  ...
  const onDrop = useCallback((acceptedFiles: File[]) => {
    ...
    if (validateFile(file)) {
      ...
      setMessage('The file is a file that can be handled by Excel.');
      ...
      setMessage("This file cannot be handled by Excel.");
  ...
  return (
    <div>
      <div {...getRootProps()} style={dropzoneStyles}>
        ...
        {message
            ? <p className='validMessage'>{message}</p>
            : <p className='failedMessage'>{message}</p>
              ?
              : null
        }
      </div>
